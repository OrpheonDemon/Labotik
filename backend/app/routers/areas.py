...
async def create_area(area_in: schemas.AreaCreate, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_laboratorista)):
    return await crud.area_crud.create(db, area_in)

@router.put("/{id_area}", response_model=schemas.AreaOut)
async def update_area(id_area: str, area_in: schemas.AreaUpdate, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_laboratorista)):
...
async def delete_area(id_area: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_laboratorista)):
