...
async def get_current_user_from_token(token: str = Depends(oauth2_scheme)):
    """
    Extrae y decodifica el token JWT sin necesidad de base de datos.
    Retorna el payload del token como diccionario.
    """
    ...
    try:
        ...
        # Mapeo correcto de rol a tabla_usuario
        rol_to_table = {
            "paciente": "pacientes",
            "medico": "medicos",
            "laboratorista": "laboratoristas",
            "administrador": "administradores",
            "recepcionista": "administradores"  # recepcionistas son administradores
        }
        tabla_usuario = rol_to_table.get(rol, rol + "s" if rol not in ["administrador", "recepcionista"] else "administradores")
        
        return {"email": email, "rol": rol, "id_usuario": id_usuario, "tabla_usuario": tabla_usuario}
    except JWTError:
        raise credentials_exception
